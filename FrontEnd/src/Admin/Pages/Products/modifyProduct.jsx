...
import { getProductById, updateProduct } from '../../Features/ProductsAdmin/Action/ProductAdminActions';
        ...
        const fetchProduct = async () => {
            try {
                ...
                reset({
                    ...
                    priceFor2: product.priceFor2,
                    priceFor3: product.priceFor3,
    ...
    const ImageDropzone = ({ index }) => {
        ...
        useEffect(() => {
            ...
            if (existingImages.length > 0) {
                const newPreviews = existingImages.map(image => ({
                    preview: typeof image === 'string' ? image : (image.webpPath ? `${API_URL}${image.webpPath}` : image.preview || URL.createObjectURL(image)),
                    isExisting: !(image instanceof File),
                    originalImage: image
                }));
        ...
        return (
            <div className="space-y-6">
                <div
                    {...getRootProps()}
                    className={`mt-6 flex justify-center px-8 py-10 border-2 ${
        ...
        <div dir='ltr' className="min-h-screen bg-gray-50 dark:bg-gray-900 py-16 px-6 sm:px-10 lg:px-16">
            <div className="max-w-6xl mx-auto">
                <div className="bg-white dark:bg-gray-800 shadow-2xl rounded-3xl p-10">
                    <h2 className="text-4xl font-bold text-gray-900 dark:text-white mb-10 border-b border-gray-200 dark:border-gray-700 pb-6">Modify Product</h2>
                                    ...
                                    <div className="flex items-center space-x-3">
                                        <button
                                            ...
                                            className={`relative inline-flex h-8 w-14 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${
                                                watch('active') ? 'bg-indigo-600 dark:bg-indigo-500' : 'bg-gray-200 dark:bg-gray-700'
                                            }`}
                                            role="switch"
                                        ...
                                        >
                                            <span
                                                ...
                                                className={`pointer-events-none inline-block h-7 w-7 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                                                    watch('active') ? 'translate-x-6' : 'translate-x-0'
                                                }`}
                                            />
                                        </button>
